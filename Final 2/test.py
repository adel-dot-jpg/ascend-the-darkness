...
# ratios of the current display size compared to the original referenced display size
# used for resizing and repositioning so that it looks the same on all display sizes.
# the original display size used for positioning and sizing was 1000x650 so thats the reference
X_SCALE = WINDOWHEIGHT/650
Y_SCALE = WINDOWWIDTH/1000
...
def test(item):
    item = pygame.image.load('assets/d.png')

def terminate():
    """closes the game when X button or ESCAPE key is pressed"""
    pygame.quit()
    os._exit(1)
...
def rescale(item, isObject):
    """resize an item and reposition it in the level to fit the current display size"""
    if isObject:
        item.left*=X_SCALE
        item.top*=Y_SCALE
        item.image=pygame.transform.scale(item.image, (item.width*X_SCALE, item.height*Y_SCALE))
    else:
        item=pygame.transform.scale(item, (item.get_rect().width*X_SCALE, item.get_rect().height*Y_SCALE))
    return item

def resize(item, isObject, x, y):
    """resize an item when the target size is known"""
    if isObject:
        item.image=pygame.transform.scale(item.image, (x, y))
    else:
        print(item, "adel")
        item=pygame.transform.scale(item, (x, y))
        print(item, "WAHHHHHHHHH")
    return item
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, image):  
        ...
        self.rect.bottom = WINDOWHEIGHT - 30 #30 is the height of the floor
...
class Game():
    ...
    def __init__(self):
        ...
        pygame.transform.scale(pimage, (pimage.get_width()*X_SCALE, pimage.get_height()*Y_SCALE))
        ...
        # set up the images
        self.pauseoverlay = pygame.transform.scale(load_image('Assets/pauseoverlay.png', True), (WINDOWWIDTH, WINDOWHEIGHT))
        self.pausedImage = pygame.transform.scale(load_image('Assets/paused.png', True), (WINDOWWIDTH, WINDOWHEIGHT))
        ...
        #print(self.floorImg.get_height)
        ...
        self.backgroundImg = pygame.transform.scale(load_image('Assets/vvallenstein.jpg', False), (WINDOWWIDTH, WINDOWHEIGHT))
        ...
        self.menuimg = pygame.transform.scale(load_image('Assets/menyou.png', False), (WINDOWWIDTH, WINDOWHEIGHT))
        self.cursorImg1 = load_image('Assets/thefinal.png', True)
        self.cursorImg2 = load_image('Assets/cursorrr.png', True)
        self.radial = pygame.transform.scale(load_image('Assets/radial2.png', True), (WINDOWWIDTH, WINDOWHEIGHT))
        self.veins = pygame.transform.scale(load_image('Assets/dein.png', True), (WINDOWWIDTH, WINDOWHEIGHT))
        ...
        #self.toResize = [self.pauseoverlay, self.pausedImage, self.backgroundImg, self.lifebar, self.hud, self.menuIcon, self.mutedIcon, self.unmutedIcon, self.artIcon, self.menuimg, self.radial, self.veins, self.blood]
        self.toRescale =[]
        self.toResize = [self.backgroundImg]
##        for item in self.toResize:
##            print(id(item))
##            print(item)
##            item = pygame.transform.scale(item, (WINDOWWIDTH, WINDOWHEIGHT))
##            print(item)
##            print(id(item)) #i have to do it separetately when it is load_image()'d ughhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh
##        print(id(self.backgroundImg))
        #    item = resize(item, False, WINDOWWIDTH, WINDOWHEIGHT)
        #self.backgroundImg = pygame.transform.scale(self.backgroundImg, (WINDOWWIDTH, WINDOWHEIGHT))
        ...
        for item in self.levelGroups[self.currentLevel]:
            item = rescale(item, True)
...
##                drawText("Click to Restart", self.SYLPH2, windowSurface, 400, 400, YELLOW)
            ...
            windowSurface.blit(self.radial, (0, 0))
...
def main():
    ...
    windowSurface = pygame.display.set_mode((0, 0), flags=pygame.FULLSCREEN)
    ...
    loading = pygame.transform.scale(load_image('Assets/loading.png', False), (WINDOWWIDTH, WINDOWHEIGHT))
